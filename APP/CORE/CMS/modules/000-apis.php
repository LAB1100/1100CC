...
class apis extends base_module {
	...
	public static function getClient($client_id) {	
		...
		$res = DB::query("SELECT
			ac.api_id, ac.name, ac.id, ac.enabled AS enabled, ac.secret AS secret, ac.user_id, ac.time_amount, ac.time_unit, ac.request_limit_disable, a.name AS api_name, ac_u.name AS user_name
	...
	public static function getClientUser($client_id, $client_user_id) {	
		...
		$res = DB::query("SELECT
			ac.api_id, acu.client_id, acu.user_id, ac.enabled AS client_enabled, ac.name AS client_name, ac.request_limit_disable AS client_request_limit_disable, acu.enabled AS enabled, acu.token, acu.date, acu.date_valid, a.name AS api_name, ac_u.name AS client_user_name, acu_u.name AS user_name
	...
	public static function getClientUserByToken($str_token) {	
		
		$res = DB::query("SELECT
			ac.api_id, acu.client_id, acu.user_id, ac.enabled AS client_enabled, ac.name AS client_name, ac.request_limit_disable AS client_request_limit_disable, acu.enabled AS enabled, acu.token, acu.date, acu.date_valid, a.name AS api_name
				FROM ".DB::getTable('SITE_API_CLIENT_USERS')." acu
				JOIN ".DB::getTable('SITE_API_CLIENTS')." ac ON (ac.id = acu.client_id)
				JOIN ".DB::getTable('SITE_APIS')." a ON (a.id = ac.api_id)
			WHERE acu.token = '".DBFunctions::strEscape($str_token)."'
		");
	...
	public static function handleClient($client_id, $enabled, $arr_client, $do_regenerate = false) {
	...
	public static function handleClientUser($client_id, $user_id, $enabled, $arr_client_user, $do_regenerate = false) {
		
		$date_valid = false;
		
		if ($arr_client_user) {
			if ($arr_client_user['date_valid']) {
				$date_valid = DBFunctions::str2DateTime($arr_client_user['date_valid'].' '.$arr_client_user['date_valid_t'], '=');
			} else if ($arr_client_user['time_amount'] && $arr_client_user['time_unit']) {
				$date_valid = DBFunctions::str2DateTime(time() + (($arr_client_user['time_amount'] * $arr_client_user['time_unit']) * 60), '=');
			}
		}
		
		if (!$user_id) {
			
			$str_token = generateRandomString(50);
			
			DB::setConnection(DB::CONNECT_CMS);
			
			$res = DB::query("INSERT INTO ".DB::getTable('SITE_API_CLIENT_USERS')." (client_id, user_id, enabled, token, date, date_valid)
					VALUES
				('".DBFunctions::strEscape($client_id)."', ".(int)$arr_client_user['user_id'].", ".DBFunctions::escapeAs($enabled, DBFunctions::TYPE_BOOLEAN).", '".DBFunctions::strEscape($str_token)."', ".DBFunctions::dateTimeNow().", ".($date_valid ? "'".$date_valid."'" : "NULL").")
			");
		} else {
			...
			if (!$arr_client_user) {
				...
				$res = DB::query("UPDATE ".DB::getTable('SITE_API_CLIENT_USERS')." SET
						enabled = ".DBFunctions::escapeAs($enabled, DBFunctions::TYPE_BOOLEAN)."
						".($do_regenerate ? "
							, token = '".DBFunctions::strEscape($str_token)."'
						" : "")."
