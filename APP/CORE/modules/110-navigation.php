...
class navigation extends base_module {
	...
	public function contents() {
		...
		if (SiteStartVars::$dir['require_login'] && SiteStartVars::$dir['user_group_id'] && SiteStartVars::$dir['user_group_id'] != $_SESSION['USER_GROUP']) {
			...
			$arr_pages = pages::filterClearance($arr_pages, $_SESSION['USER_GROUP'], $_SESSION['CUR_USER'][DB::getTableName('TABLE_USER_PAGE_CLEARANCE')]);
			...
			foreach ($arr_directories as $arr_directory) {
				...
				if ($cur_path_length != $arr_directory['path_length']) {
					...
					foreach ($arr_pages as $arr_page) {
						...
						if ($arr_page['directory_id'] == $arr_directory['ancestor_id']) {
							...
							$return .= '<li'.($arr_directory['path_length'] == 0 && $arr_page['id'] == SiteStartVars::$page['id'] ? ' class="active"' : '').'><a href="'.($arr_page['url'] ? htmlspecialchars($arr_page['url']) : $base_url.$arr_page['name']).'">'.htmlspecialchars(Labels::parseTextVariables($arr_page['title'])).'</a></li>';
				...
				$return .= '<li'.($arr_active[$arr_directory['id']] || $arr_directory['id'] == SiteStartVars::$dir['id'] ? ' class="active"' : '').'><a href="'.$base_url.$arr_directory['name'].'/">'.htmlspecialchars(Labels::parseTextVariables($arr_directory['title'])).'</a></li>';
			...
			if (count($arr_pages)) {
				...
				foreach ($arr_pages as $arr_page) {
					...
					$cur_pages .= '<li'.($arr_page['id'] == SiteStartVars::$page['id'] ? ' class="active"' : '').'><a href="'.($arr_page['url'] ? htmlspecialchars($arr_page['url']) : $base_url.$arr_page['name']).'">'.htmlspecialchars(Labels::parseTextVariables($arr_page['title'])).'</a></li>';
				}
				if ($cur_pages) {
