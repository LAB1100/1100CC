...
// 1100CC Framework:
	...
	if (SiteStartEnvironment::getAPI()) {
		...
		$check = Log::checkRequest($str_request_identifier, null, (($arr_api['request_limit_amount'] * $arr_api['request_limit_unit']) * 60), ['ip' => $arr_api['request_limit_ip'], 'ip_block' => ($arr_api['request_limit_ip'] * 4), 'global' => $arr_api['request_limit_global']]);
		...
		if ($_SERVER['HTTP_AUTHORIZATION']) {
			...
			try {
				...
				if ($arr_method_token[0] == 'Bearer' && $arr_method_token[1]) {
					...
					error(Labels::getSystemLabel('msg_request_invalid_authentication'), TROUBLE_INVALID_REQUEST,
	...
	if (SiteStartEnvironment::getContext(SiteStartEnvironment::CONTEXT_PAGE_KIND) == '.e') {
		...
		if (!isset($obj->message)) {
			...
			message(getLabel('lbl_not_available'), 'SORRY', LOG_CLIENT);
			Log::addToObject($obj);
		}
		
		$page = new ExitPage($obj->message, SiteStartEnvironment::getContext(SiteStartEnvironment::CONTEXT_PAGE_NAME), SiteStartEnvironment::getContext(SiteStartEnvironment::CONTEXT_PAGE_NAME));
	...
	if (SiteStartEnvironment::getContext(SiteStartEnvironment::CONTEXT_PAGE_KIND) == '.manifest') {
	...
	if (SiteStartEnvironment::getContext(SiteStartEnvironment::CONTEXT_PAGE_KIND) == '.p') {
